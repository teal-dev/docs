...
description: "This page walks you through configuring the chart of accounts for your instances."
icon: "gear"
---

## What is a chart of accounts

Every instance needs to have a chart of accounts (CoA). A chart of accounts is the full set of ledgers that a 
instance has access to when categorizing transactions or creating journal entries. 
...
You can manage CoA templates and the ledgers associated with each via the [Chart of Accounts Templates endpoint](/api-reference/chart-of-accounts-templates/get-coa-templates), or 
...
Once an instance is created, they will have every ledger from their specified CoA template available to them. 
Future changes to the template will not affect existing instances.

<Info>
When a instance is created with a default CoA, the ledger names will be identical to the ledger names in 
the default companies, however, every instance's ledger "ids" are unique. 
These "ids" are not shared between instances.
</Info>

Instances will need some additional instance-specific ledgers beyond the default CoA in order to do their bookkeeping. 
...
the [Create Ledger endpoint](/api-reference/ledgers/create-ledger). See the guide 
on [creating ledgers](/accounting_package/platform_gl/transactions/create_ledger) for more information.

In addition, you may opt to allow users to create their own custom ledgers beyond the 
defaults you have provided. You can use the [Create Ledger endpoint](/api-reference/ledgers/create-ledger) 
