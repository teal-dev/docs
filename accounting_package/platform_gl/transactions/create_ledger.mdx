...
account that the transactions will belong to. We support multiple types of financial accounts and 
set smart defaults based on the `financial_account_type` selected. For example, if you set 
`financial_account_type` to bank, we set `type` to asset, `debit_credit` to debit, and 
`report_cash_flow` to true. Each financial acount should have its own individual ledger. You can see the 
complete list of supported financial account types [here](/api-reference/ledgers/create_ledger).
...
When you first [create an instance](/api-reference/instances/create-instance) in Teal, you need to specify the user's start date (`entries_start`). No 
transactions are allowed to be entered into Teal before this start date. If the financial 
account has been open before the user's start date, then it's important for the 
[opening balance](/accounting_package/platform_gl/transactions/opening_balances) on the Teal ledger to match the balance 
in the financial account on the start date. There is a dedicated feature for opening balances 
coming soon, however, until then you can simply create a journal entry on the start date with 
the title "Opening balance", and debit/credit the correct amount to the ledger so that the 
balance is correct on the user's start date. This will ensure that later, the balance will 
...
a `sort_code` appropriate to the kind of financial account it is.

If you are using Teal's default chart of accounts, here are the recommended sort codes for the 
major financial account types:
...
source (see [Data Integrations Configuration](/accounting_package/platform_gl/configuration/data_sources_config))
2. Create a ledger for the financial account by using the 
[Create Ledger endpoint](/api-reference/ledgers/create-ledger). For bank accounts, you 
...
3. Link the user's financial account to this ledger by using the [Link Unit Account endpoint](/api-reference/transaction-sources/link-unit-account).
