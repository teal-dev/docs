...
<Tabs>
  <Tab title="Plaid">
    ...
    Through Teal's [official processor](https://plaid.com/docs/auth/partnerships/teal/) partnership with [Plaid](), your users can connect their external financial accounts with Teal using your business' Plaid account and custom UX. Syncing accounts imports two years of data and keeps the account up to date daily.
    ...
    This guide walks you through the steps necessary to set up Plaid and how to pass that information back to Teal as well as how to connect to our webhooks so your customers can diagnose and respond to events from Plaid.

    #### Prerequisites

    This guide assumes you have both Teal and Plaid accounts set up.

    <Steps>
      <Step title="Enable the Plaid-Teal integration">
        The Plaid-Teal integration can be found [here]() in your Plaid dashboard. Ensure that it is set to Enabled.
      </Step>
      <Step title="Build your Plaid-Teal integration">
        You will need to use Plaid Link to give authorization for Plaid to access their accounts. Follow the  [Plaid-Teal integration guide](https://plaid.com/docs/auth/partnerships/teal/#create-a-link_token) which walks you through the steps necessary. To summarize, there are 4 steps involved:


        1. Request a `link_token`to allow the user to use Plaid Link.
        2. After the user completes the Plaid Link flow, exchange the resulting `public_token` for an `access_token`.
        3. Store your `access_token` and use it to request a `processor_token`.
        4. Send the `processor_token` to Teal.

        After Teal has saved the `processor_token`, Teal will automatically create a new ledger for the financial account and sync 2 years of transactions, if they're available. It sometimes takes a few minutes for all of the transactions to be processed. 
        ...
        <Note> 
          Plaid supports three environments: sandbox, development, and production. All three can generate `processor_tokens`; to work with real transactions, we require you generate your production `processor_token`.
        </Note>
