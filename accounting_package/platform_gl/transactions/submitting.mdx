...
Once you have [configured an external data source](/accounting_package/platform_gl/configuration/data_sources_config) in the developer portal, you can link a ledger to this data source using the [Link Plaid Account endpoint](/api-reference/transaction-sources/link-plaid-account).
...
At any time you can use the [Get User Ledger Line Entries endpoint](/api-reference/line-entries/get-ledger-line-entries) to get a list of the line entries that have been created for a ledger. Each entry in the list will contain the transaction ID you submitted on creation, allowing you to compare the list of line entries to the list of transactions to compare them. 
