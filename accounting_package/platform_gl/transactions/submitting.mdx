...
Before you can submit transactions, you must have first set up an appropriate financial account ledger for the transactions that
 ...
 first go [complete this step.](/accounting_package/platform_gl/transactions/create_ledger)
...
There are a two ways to submit transactions to Teal, either using the [Create Transactions endpoint](/api-reference/transactions/create-transactions), or by configuring an external data source.

If you would like more direct control over the accounting treatment, it is possible to create journal entries directly by calling the [Create Journal Entry endpoint](/api-reference/journal-entries/create-journal-entry). However, this is not recommended unless you have very high certainty in your accounting treatment and are trying to solve a specific use case.
...
The advantage of linking an external data source, rather than using [Create Transactions endpoint](/api-reference/transactions/create-transactions), is that Teal does the work of making sure the list of transactions is complete and correct.

Once you have [configured an external data source](/accounting_package/platform_gl/configuration/data_sources_config) in the developer portal, you can link a ledger to this data source using the [Transaction Sources endpoint](/api-reference/transaction-sources/create-plaid-link).
...
When a new transaction is submitted via the [Create Transactions endpoint](/api-reference/transactions/create-transactions) or fetched by Teal via a linked external source, Teal will automatically apply your [categorization pipeline](/accounting_package/platform_gl/configuration/categorization_config) to the new transaction, to automatically book the transaction to an appropriate ledger.

#### Submitting metadata

When submitting transactions, you can optionally include metadata with the transaction using the metadata parameter on the [Create Transactions endpoint](/api-reference/transactions/create-transactions). This is an opportunity to submit additional data that you would like the transaction to be categorized based on, such as the location of the transaction, the vendor name or type, or the user of the card making the transaction. This metadata will be available to be used by your auto-categorization pipeline to create more specific and accurate transaction categorizations.
...
Teal takes your submitted transactions and transactions fetched from external sources, and turns them into appropriate line entries and journal entries in the accounting database. You can learn more about journal entries and line entries in our [accounting core concepts guide](/accounting_package/platform_gl/core_accounting_concepts).

At any time you can use the [Get User Ledger Line Entries endpoint](/api-reference/line-entries/get-filtered-line-entries) to get a list of the line entries that have been created for a ledger. Each entry in the list will contain the transaction ID you submitted on creation, allowing you to compare the list of line entries to the list of transactions to compare them. 
