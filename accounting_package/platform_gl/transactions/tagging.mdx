...
Both the [Create Journal Entry](/api-reference/journal-entries/create-journal-entry) and [Update Journal Entry](/api-reference/journal-entries/update-journal-entry) endpoints accept a parameter `tag_ids`. Using either of these methods will apply the Tags to all Line Entries in the Journal Entry.
...
Use the [Update Line Entry](/api-reference/line-entries/update-line-entry) endpoint to update the Tags on a specific Line Entry.
