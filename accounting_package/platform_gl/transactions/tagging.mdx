...
Tags are fundamentally an attribute of Line Entries, but because users tend to think of line entries and transactions interchangably, we provide a few ways of accessing Tags: via Transaction, Journal Entry, and Line Entry endpoints.

Tags can also be added via each of these endpoints, but each method has important and different results.

#### Tagging Transactions

You can assign Tags via the [Create Transactions](/api-reference/transactions/create-transactions) and the [Update Transaction](/api-reference/transactions/update-transaction) endpoints by passing ids of each tag into `tag_ids`. 

Using either of these methods will apply the tag to all of the Line Entries connected to the Transaction. 

#### Tagging Journal Entries

Both the [Create Journal Entry](/api-reference/journal-entries/create-journal-entry) and [Update Journal Entry](/api-reference/journal-entries/update-journal-entry) endpoints accept a parameter `tag_ids`. 

When a Journal Entry is processed through either method, these Tags will be applied to all Line Entries included in the Journal Entry.
