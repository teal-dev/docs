...
Reviewing Transactions is a key part of building an accounting package for your users as it allows them to provide input on Transaction's categories, ensuring that ledgers are correctly balanced. Use a combination of the [Transactions](/api-reference/transactions/get-categorized-transactions) and [Ledger](/api-reference/transactions/get-ledgers) endpoints to enable users to review transactions. There are several use cases:

- Check that transactions have been categorized properly
- Categorize transactions that have been assigned the "uncategorized inflow" or "uncategorized outflow" category by the auto-categorizer (See notes)
...
<Steps>
	...
	<Step title="Save the updated Transaction">
		When you're ready, update the Transaction `review_status` from `unreviewed` to `reviewed` using the [Update Transaction Status](/api-reference/transactions/update-transaction-status) endpoint with `ledger_id` and `transaction_id`, passing along the new review status.
